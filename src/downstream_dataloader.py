# Script for loading all classification datasets
# Adapted from: https://github.com/vishaal27/SuS-X/blob/main/dataloader.py
...
class DataLoader():
    def __init__(self, args, preprocess, data_root=None):
        # Method signature:
        #   args: Args for loading dataset
        #   preprocess: Image transform function before converting to tensors
        #   data_root: Path to dataset root directory
        dp = data_root if data_root is not None else DATASET_PATH.format(args.dataset)
    ...
    def load_dataset(self):
        ...
        elif(self.args.dataset == 'let_it_wag_combined'):
            return self.let_it_wag_combined()
        else:
    ...
    def country_211_load(self):
        ...
        return val_images, val_loader, test_images, test_loader, num_classes, string_classnames
    ...
    def cifar10_load(self):
        ...
        print('Load CIFAR10 data finished.')

        # For CIFAR-10 the test and val sets are the same -- hence returning same sets for both
        return testset, testloader, testset, testloader, num_classes, string_classnames
    ...
    def cifar100_load(self):
        ...
        num_classes = len(string_classnames)
        print('Load CIFAR100 data finished.')
    ...
    def imagenet_load(self):  
        ...
        return val_images, val_loader, val_images, val_loader, num_classes, string_classnames

    def let_it_wag_combined(self):
        ...
        # save file paths
        classnames = sorted(list(set([
            '_'.join(sample[0].split('/')[-2].split('_')[:-1]) 
            if sample[0].split('/')[-2].split('_')[-1] in ['bird', 'flowers', 'aircraft', 'food'] 
            else sample[0].split('/')[-2] 
            for sample in val_images.samples
        ])))

        print('Load LetItWag-Combined data finished.') 

        return val_images, val_loader, val_images, val_loader, num_classes, classnames
    ...
    def custom_load(self):
        ...
        # for now only returning val and test sets
        return val_dataset, val_loader, test_dataset, test_loader, num_classes, string_classnames
...
if __name__ == '__main__':
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 50000 and len(test_images) == 50000
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 10000 and len(test_images) == 10000
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 10550 and len(test_images) == 21100
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 7774 and len(test_images) == 11747
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 1635 and len(test_images) == 8041 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 1898 and len(test_images) == 3783
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 1649 and len(test_images) == 2465 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 6027 and len(test_images) == 9076 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 1194 and len(test_images) == 5794 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 1633 and len(test_images) == 2463 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 3970 and len(test_images) == 19850 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 1128 and len(test_images) == 1692 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 5400 and len(test_images) == 8100 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 3333 and len(test_images) == 3333 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 736 and len(test_images) == 3669 
    ...
    k_shot_dl = DataLoader(args, preprocess)
    val_images, val_loader, test_images, test_loader, num_classes, string_classnames = k_shot_dl.load_dataset()
    assert len(val_images) == 20200 and len(test_images) == 30300 
